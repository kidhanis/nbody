...
void GlutWrapper::init(int argc, char **argv, int theWidth, int theHeight, Shaders *shaders, float*&all, size_t bufSize, float *buf) {

	_width = theWidth; // HEIGHT AND WIDTH OF WINDOW
	_height = theHeight;
	if( _windowTitle == "" ) {
  ...
  if( _debugMode == Mode::DEBUG ) {
	...
	initVertexBuffers( bufSize, buf, all ); //INFO SENT BY MAIN
	glGenVertexArrays( 1, &_vao );
}

void GlutWrapper::createWindow() {
  glutInitWindowSize( _width, _height );// SIZE OF WINDOW
  _windowId = glutCreateWindow( _windowTitle.c_str() );// NAME OF WINDOW
...
void GlutWrapper::initVertexBuffers( size_t bufSize, float *buf , float*&all) {
...
void GlutWrapper::run() {
	...
	glutDisplayFunc( displayCallback );// EVERYTHING SHOWN IN THE WINDOW
	glutReshapeFunc( reshapeCallback );// NOT ACCESSED AT BEGINNING.  DON'T KNOW ITS FUNCTION YET
