...
void Shaders::addToVertexList( std::string shaderFile ) {
	_vertexShaderText.push_back( shaderFile ); // FIRST STRING IN TOP OF MAIN
}

void Shaders::addToFragmentList( std::string shaderFile ) {
	_fragmentShaderText.push_back( shaderFile );//SECOND STRING IN TOP OF MAIN
}

GLuint Shaders::build() {
	for( auto vertexShader : _vertexShaderText ) { //TAKES  STRINGS AND PASS TO COMPILE; ADD TO NEW LIST
